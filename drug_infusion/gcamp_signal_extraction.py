...
from common.mask.neuropil_mask import roi_map_to_neuropil_masks


#%% main 
OUT_DIR_RAW_DATA = r'Z:\Jingyu\LC_HPC_manuscript\raw_data\drug_infusion\raw_signals'
# for ss_idx, date_ss in selected_rec_lst.iloc[10:41].iterrows():
# for ss_idx, date_ss in selected_rec_lst.iterrows().heads(-1):
for ss_idx, date_ss in selected_rec_lst.loc[(selected_rec_lst['anm']=='AC310')&
                                            (selected_rec_lst['date']=='20250828')].iterrows():
    temps = []
    ...
    if os.path.exists(os.path.join(INT_PATH, "sig_master_raw_new.nc")) & os.path.exists(os.path.join(INT_PATH, "sig_master_neu_raw.nc")):
        ...
        try:
            shift_ds.to_netcdf(os.path.join(INT_PATH, "shift_ds_new.nc"))
        ...
        # extract neuropil masks first (needed for the combined loop)
        ...
        A_neuropil.values = neuropil_map.reshape(A_master.shape).astype(float)
        # Save to netCDF file in the same directory as A_master
        A_neuropil.to_netcdf(os.path.join(INT_PATH, "A_master_neu.nc"))

        # extract ROI and neuropil signals together (single loop to avoid reloading dat)
        for (anm, Am), (_, Am_neu) in zip(A_master.groupby("animal"), A_neuropil.groupby("animal")):
            sigs = []
            ...
            for ss in sessions:
                ...
                # extract neuropil signals (using same dat)
                curA_neu = apply_transform(Am_neu, -sh)
                neu_pix_count = curA_neu.sum(dim=["height", "width"])
                cur_sig_neu = curA_neu.dot(dat).compute() / neu_pix_count
