...
def calculate_ratio(df_cal_profile_all, s, session_info, trace_type, beh, 
    ...
    # df_cal_profile_all[f"run_cal_profile_all_rsd_{s}"] = [nd_to_list(session_array_filtered[i]) for i in range(len(df_cal_profile_all))]
...
def identify_cell_response(rec, df_cal_profile_all, session_info,
                           trial_bef=2, trial_aft=4, pre_window=(-1, 0), post_window=(0, 1.5)):
    # filter interneuron, small pieces ROIs and inactive ROIs
    ...
    for i, s in enumerate(sessions):
        ...
        df_cal_profile_all = calculate_ratio(df_cal_profile_all, s, session_info, trace_type='raw', beh=beh,
    ...
    # ax.hist(df_cal_profile_all['baseline_mean_ss1']-df_cal_profile_all['baseline_mean_ss2'], bins=100)
    # ax.set(xlabel='baseline_mean ss1 - ss2')
    
        # if shuffle:
        session_array = np.stack([np.stack(r) for r in df_cal_profile_all[f"run_cal_profile_all_rsd_{s}"]])
        shuffle_means_all = circular_shuffle_mean_session(session_array, shuff_times=1000, gpu=1)
        df_cal_profile_all[f'shuffle_means_{s}'] = ''
        df_cal_profile_all[f'shuffle_means_95_{s}'] = ''
        df_cal_profile_all[f'shuffle_means_5_{s}'] = ''
        df_cal_profile_all[f'shuffle_means_mean_{s}'] = ''
        df_cal_profile_all[f'shuffle_means_{s}'] = [nd_to_list(row) for row in shuffle_means_all]
        df_cal_profile_all[f'shuffle_means_95_{s}'] = [np.percentile(row, 95, axis=0) for row in shuffle_means_all]
        df_cal_profile_all[f'shuffle_means_5_{s}'] = [np.percentile(row, 5, axis=0) for row in shuffle_means_all]
        df_cal_profile_all[f'shuffle_means_mean_{s}'] = [np.nanmean(row, axis=0) for row in shuffle_means_all]
        # identify if shows significant run-onset tunning
        
        # thresh_up = df_cal_profile_all['shuffle_ratios_95']
        # thresh_down = df_cal_profile_all['shuffle_ratios_5']
        
        # check run-osnet tunning
        df_cal_profile_all[f'run_tunned_higher_{s}'] = ''
        df_cal_profile_all[f'run_tunned_lower_{s}'] = ''
        for roi_idx, df_roi in df_cal_profile_all.iterrows():
            up_intervals = utl.numOfConsecutiveOnes(df_roi[f'run_cal_profile_rsd_{s}']>df_roi[f'shuffle_means_95_{s}'])
            down_intervals = utl.numOfConsecutiveOnes(df_roi[f'run_cal_profile_rsd_{s}']<df_roi[f'shuffle_means_5_{s}'])
            ...
            # test plot
            # fig, ax = plt.subplots()
            # ax.plot(df_roi[f'run_cal_profile_rsd_{s}'], color='green')
            # ax.plot(df_roi[f'shuffle_means_95_{s}'], color='gray')
            # ax.plot(df_roi[f'shuffle_means_5_{s}'], color='gray')
            if np.max(up_intervals)>15:
                df_cal_profile_all.loc[roi_idx, f'run_tunned_higher_{s}'] = 1
            else:
                df_cal_profile_all.loc[roi_idx, f'run_tunned_higher_{s}'] = 0
            if np.max(down_intervals)>15:
                df_cal_profile_all.loc[roi_idx, f'run_tunned_lower_{s}'] = 1
            else:
                df_cal_profile_all.loc[roi_idx, f'run_tunned_lower_{s}'] = 0
...
#%%    
    ...
    INT_PATH = r"Z:\Jingyu\2P_Recording\{}\{}\concat_anat_drd1".format(anm_id, anm_id+'-'+date)
    p_df_profile = INT_PATH+r"\{}_{}_df_cal_profile_all_valid_trials_drd1_only.parquet".format(anm_id, date)
