...
from . import Regression_Red_From_Green_ROIs_geco
...
def _correct_single_roi_worker(
    ...
    if not valid_event_frames:
        ...
        return {'corrected': np.full(T, np.nan), 'artifact': np.full(T, np.nan), 'qc': None}

    # --- Initialize QC lists to collect regression metrics across all events ---
    r2_list = []
    b1_list = []
    b2_list = []
    event_frame_list = []
    start_frame_list = []
    end_frame_list = []
    n_valid_frames_list = []  # Track how many valid frames were used for fitting
    ...
    for i, onset in enumerate(valid_event_frames):
        ...
        try:
            if valid_mask is not None and n_valid_frames < 100:
                ...
                fit_start_frame = start_frame
                fit_end_frame = end_frame
                n_fit_frames = n_valid_frames
                ...
                for frame_idx in range(end_frame, next_segment_end):
                    if not red_trace_mask[frame_idx]:  # This frame is valid
                        n_fit_frames += 1
                    fit_end_frame = frame_idx + 1
                    if n_fit_frames >= 100:
                        break

                # --- Step 2: If still < 100, borrow from PREVIOUS segment ---
                if n_fit_frames < 100 and i > 0:
                    ...
                    for frame_idx in range(start_frame - 1, prev_segment_start - 1, -1):
                        if not red_trace_mask[frame_idx]:  # This frame is valid
                            n_fit_frames += 1
                        fit_start_frame = frame_idx
                        if n_fit_frames >= 100:
                            break

                # --- Step 3: Use whatever valid frames we have (even if < 100) ---
                # Extract extended slices for fitting only
                y_fit_extended = orig_green_trace[fit_start_frame:fit_end_frame]
                neuropil_extended = green_bg_trace[fit_start_frame:fit_end_frame]
                red_extended = red_trace[fit_start_frame:fit_end_frame]
                mask_extended = ~red_trace_mask[fit_start_frame:fit_end_frame]  # valid = True
                ...
                # Calculate R2 on the extended fitting range
                ...
                # QC: fitting range is the current segment, valid frames only
                fit_start_frame = start_frame
                fit_end_frame = end_frame
                n_fit_frames = n_valid_frames

                # Calculate R2 on the valid frames used for fitting
                r2 = model.score(X_fit, y_fit)
                ...
                # QC: fitting range is the current segment, all frames
                fit_start_frame = start_frame
                fit_end_frame = end_frame
                n_fit_frames = len(y_target)

                # Calculate R2 on all frames
                r2 = model.score(X_regressors, y_target)
            ...
            # --- Collect QC metrics for this event ---
            r2_list.append(r2)
            b1_list.append(b1)
            b2_list.append(b2)
            event_frame_list.append(onset)
            start_frame_list.append(fit_start_frame)
            end_frame_list.append(fit_end_frame)
            n_valid_frames_list.append(n_fit_frames)
    ...
    # Build QC dictionary for this ROI
    qc_dict = {
        'r2': r2_list,
        'b1': b1_list,
        'b2': b2_list,
        ...
        'n_valid_frames': n_valid_frames_list
    }
...
def run_red_channel_correction_single_trials_parallel(
    ...
    for i, res_dict in enumerate(results):
        if res_dict is not None:
            ...
            # Add ROI index to QC dict and collect
            if res_dict.get('qc') is not None:
                qc = res_dict['qc'].copy()
                qc['roi_idx'] = i
                all_qc_dicts.append(qc)

    return corrected_traces_out, estimated_artifact, all_qc_dicts
...
def run_and_save_motion_correction_results(
    ...
    # output_path = output_path / regression_name
    output_path.mkdir(parents=True, exist_ok=True)
    
    # --- STEP 1: Perform regression for each ROI in parallel ---
    print("\nStep 1: Performing parallel regression for all ROIs...")
    corrected_dlight, red_regressor, all_qc_dicts = run_red_channel_correction_single_trials_parallel(
    ...
    Regression_Red_From_Green_ROIs_geco.run_dashboard_creation_parallel(
        ...
        pre_frames=pre_frames,
        post_frames=post_frames,
        output_dir=fig_out_dir, # Save figures in the dedicated subdirectory
        n_jobs=n_jobs,
        num_rois_to_plot=num_rois_to_plot # plot first 50 ROIs
    )

    # --- STEP 3: Save all regression results to a single file ---
    data_save_path = output_path / f'{regression_name}_res_traces.npz'
    ...
    # --- STEP 4: Save regression QC data as pickle ---
    if all_qc_dicts:
        qc_save_path = output_path / f'{regression_name}_qc.pkl'
        print(f"\nStep 4: Saving regression QC data to: {qc_save_path}")
        regress_qc_df = pd.DataFrame(all_qc_dicts)
        regress_qc_df.to_pickle(qc_save_path)
        print("QC data save complete.")

    print(f"\n--- Pipeline Complete. Results saved in {output_path} ---")

    # Return the key output data for immediate use
    return corrected_dlight #red_regressor, all_qc_dicts
