...
def quantify_event_response(corrected_traces, event_frames,
    ...
    if  shuffle_test:
        ...
        shuffled_segments_valid = cp.zeros((n_valid_rois, n_trials, trial_length))
        ...
        for shuffle_idx in tqdm(range(n_shuffle), desc='Using GPU for shuffle...'):
            ...
            # Vectorized roll operation for all trials
            for trial_idx in range(n_trials):
                shuffled_segments_valid[:, trial_idx, :] = cp.roll(
                    valid_trial_aligned[:, trial_idx, :],
                    int(shifts[trial_idx]),
                    axis=1
                )

            # Extract baseline and response segments (vectorized slicing)
            ...
            # Calculate means across time dimension (vectorized)
            ...
            shuffle_pooled_stds_valid[:, shuffle_idx] = calculate_pooled_std_cp(shuffled_baseline_segments,
                                                                          shuffled_response_segments) # (n_valid_rois, n_shuffle)
